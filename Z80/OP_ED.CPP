...
Z80OPCODE ope_57(Z80 *cpu) { // ld a,i
   ...
   if (cpu->iff1 && (cpu->t+10 < conf.frame)) cpu->f |= PV;
   cpu->t++;
}
#endif
#ifndef Z80_COMMON
...
Z80OPCODE ope_5F(Z80 *cpu) { // ld a,r
   ...
   cpu->f = (log_f[cpu->a] | (cpu->f & CF)) & ~PV;
   if (cpu->iff2 && ((cpu->t+10 < conf.frame) || cpu->eipos+8==cpu->t)) cpu->f |= PV;
   cpu->t++;
}
#endif
#ifndef Z80_COMMON
...
Z80OPCODE ope_67(Z80 *cpu) { // rrd
   unsigned char tmp = rm(cpu->hl);
   cpu->memptr = cpu->hl+1;
   wm(cpu->hl, (cpu->a << 4) | (tmp >> 4));
...
Z80OPCODE ope_70(Z80 *cpu) { // in (c)
   cpu->t += 4;
   cpu->memptr = cpu->bc+1;
   cpu->f = log_f[in(cpu->bc)] | (cpu->f & CF);
}
...
Z80OPCODE ope_A0(Z80 *cpu) { // ldi
   cpu->t += 8;
   unsigned char tempbyte = rm(cpu->hl++);
   wm(cpu->de++, tempbyte);
...
Z80OPCODE ope_B8(Z80 *cpu) { // lddr
   ...
   tempbyte += cpu->a; tempbyte = (tempbyte & F3) + ((tempbyte << 4) & F5);
   cpu->f = (cpu->f & ~(NF|HF|PV|F3|F5)) + tempbyte;
   if (--cpu->bc16) cpu->f |= PV, cpu->pc -= 2, cpu->t += 5;
}
