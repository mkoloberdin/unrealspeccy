...
Z80OPCODE op_10(Z80 *cpu) { // djnz rr
   if (--cpu->b) {
      signed char offs = (char)rm(cpu->pc);
...
Z80OPCODE op_34(Z80 *cpu) { // inc (hl)
   unsigned char hl = rm(cpu->hl);
   inc8(cpu, hl);
//   wm(cpu->hl, hl);
   cpu->t += 7;
   wm(cpu->hl, hl); //Alone Coder
}
...
//   wm(cpu->hl, rm(cpu->pc++));
   cpu->t += 6;
   wm(cpu->hl, rm(cpu->pc++)); //Alone Coder
}
#endif
#ifdef Z80_COMMON
...
//   wm(cpu->hl, cpu->b);
   cpu->t += 3;
   wm(cpu->hl, cpu->b); //Alone Coder
}
Z80OPCODE op_71(Z80 *cpu) { // ld (hl),c
//   wm(cpu->hl, cpu->c);
   cpu->t += 3;
   wm(cpu->hl, cpu->c); //Alone Coder
}
Z80OPCODE op_72(Z80 *cpu) { // ld (hl),d
//   wm(cpu->hl, cpu->d);
   cpu->t += 3;
   wm(cpu->hl, cpu->d); //Alone Coder
}
Z80OPCODE op_73(Z80 *cpu) { // ld (hl),e
//   wm(cpu->hl, cpu->e);
   cpu->t += 3;
   wm(cpu->hl, cpu->e); //Alone Coder
}
Z80OPCODE op_74(Z80 *cpu) { // ld (hl),h
//   wm(cpu->hl, cpu->h);
   cpu->t += 3;
   wm(cpu->hl, cpu->h); //Alone Coder
}
Z80OPCODE op_75(Z80 *cpu) { // ld (hl),l
//   wm(cpu->hl, cpu->l);
   cpu->t += 3;
   wm(cpu->hl, cpu->l); //Alone Coder
}
#endif
#ifdef Z80_COMMON
...
Z80OPCODE op_C0(Z80 *cpu) { // ret nz
   if (!(cpu->f & ZF)) {
      unsigned addr = rm(cpu->sp++);
      addr += 0x100*rm(cpu->sp++);
      cpu->pc = addr;
      cpu->memptr = addr;
      cpu->t += 7;
   } else cpu->t += 1;
...
Z80OPCODE op_C2(Z80 *cpu) { // jp nz,nnnn
   cpu->t += 6;
   unsigned addr = rm(cpu->pc);
   addr += 0x100*rm(cpu->pc+1);
   cpu->memptr = addr;
   if (!(cpu->f & ZF)) {
      cpu->pc = addr;
   } else cpu->pc += 2;
...
Z80OPCODE op_C3(Z80 *cpu) { // jp nnnn
   unsigned lo = rm(cpu->pc++);
   cpu->pc = lo + 0x100*rm(cpu->pc);
   cpu->memptr = cpu->pc;
   cpu->t += 6;
}
Z80OPCODE op_C4(Z80 *cpu) { // call nz,nnnn
   cpu->pc += 2;
   unsigned addr = rm(cpu->pc-2);
   addr += 0x100*rm(cpu->pc-1);
   cpu->memptr = addr;
   if (!(cpu->f & ZF)) {
      wm(--cpu->sp, cpu->pch);
...
Z80OPCODE op_D3(Z80 *cpu) { // out (nn),a
   unsigned port = rm(cpu->pc++);
   cpu->t += 7;
   cpu->memptr = ((port+1) & 0xFF) + (cpu->a << 8);
   out(port + (cpu->a << 8), cpu->a);
}
...
Z80OPCODE op_DB(Z80 *cpu) { // in a,(nn)
   unsigned port = rm(cpu->pc++) + (cpu->a << 8);
